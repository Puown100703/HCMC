...
const FormTemplateForm = () => {
  ...
  const onFinish = async (values) => {
    try {
      ...
      console.error(`Error ${isEditMode ? "updating" : "creating"} form template:`, err);
      message.error(
        `Không thể ${isEditMode ? "cập nhật" : "thêm"} mẫu biểu. Vui lòng thử lại sau.`
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="form-template-form">
      <Breadcrumb
        items={[
          { title: "Trang chủ" },
          { title: "Quản trị" },
          { title: <a onClick={() => navigate("/admin/form-templates")}>Mẫu biểu</a> },
              ...
              <Form.Item label="Xem trước HTML">
                ...
                <div className="mt-2">
                  <Button 
                    type="default" 
                    icon={<EyeOutlined />}
                    onClick={() => navigate(`/admin/form-templates/${id}/preview`)}
                  >
                    Xem đầy đủ
                  </Button>
                </div>
